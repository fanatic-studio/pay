...
- (void)onLog:(NSString*)log logLevel:(WeChatLogLevel)level;
...
+ (void)startLogByLevel:(WeChatLogLevel)level logBlock:(WXLogBolock)logBlock;
 ...
 */
+ (void)startLogByLevel:(WeChatLogLevel)level logDelegate:(id<WXApiLogDelegate>)logDelegate;
